...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(self, email, username, first_name, last_name, password,  **extra_fields):
        ...
        if not first_name:
            raise ValueError(_("You must enter a first_name"))

        if not last_name:
            raise ValueError(_("You must enter a second_name"))

        if email:
            email = self.normalize_email(email)
            self.email_validator(email)
        else:
            raise ValueError(_('Base User Account: You must provide a email address'))
